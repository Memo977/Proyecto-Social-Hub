...
class PostController extends Controller
{
    ...
    public function update(Request $request, Post $post)
    {
        ...
        // Si entre los destinos viene Reddit, obligamos sus campos (igual que store())
        $selectedIds = $validated['targets'] ?? [];
        if (!empty($selectedIds)) {
            $selectedAccounts = SocialAccount::whereIn('id', $selectedIds)->get();
            $wantsReddit = $selectedAccounts->contains(fn($a) => $a->provider === 'reddit');
        ...
        // === Actualizar campos base y modo (igual que store()) ===
        $post->fill([
            'content'   => $validated['content'],
            'title'     => $validated['title'] ?? null,
            'media_url' => $validated['media_url'] ?? null,
        ...
        // === Meta de Reddit (mismo tratamiento que store()) ===
        $sr   = $this->normalizeSr($request->input('reddit_subreddit'));
        $kind = $request->input('reddit_kind', 'self');
        ...
        // === Re-sincronizar destinos como en store() ===
        // Si el form trae targets[], reemplazamos; si no, conservamos los existentes.
        if ($request->has('targets')) {
            $rawTargets = $request->input('targets', []);
            if (!is_array($rawTargets)) {
                $rawTargets = [$rawTargets];
            }
            $targetIds = collect($rawTargets)->map(fn($v) => (int) $v)->unique()->values();

            // Borramos y recreamos para que queden alineados con lo marcado
            $post->targets()->delete();
            ...
            // Si no cambió la selección, nos aseguramos que estén "pending" para re-publicar si corresponde
            $post->targets()->update(['status' => 'pending']);
        }

        // Siempre trabajar con targets frescos (evita colección en caché)
        $targets = $post->targets()->with('socialAccount')->get();

        // === Ramas idénticas a store(): schedule / queue / now ===
        if ($validated['mode'] === 'schedule') {
            $when = null;
        ...
        // now => inmediato a TODOS los destinos
        foreach ($targets as $t) {
            PublishPost::dispatch($post->id, $t->id);
        }
